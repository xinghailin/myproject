...
public class AlipayNotify {
	...
	static private Logger logger = LoggerFactory.getLogger(AlipayNotify.class);
	...
	public static boolean verify(Map<String, String> params) {
		logger.debug("开始进行支付宝回调有效性验证, 回调参数:{}", params);
		...
		if (isSign
		// &&
		// responseTxt.equals("true")//不进行验证了可以手动验证格式如：https://mapi.alipay.com/gateway.do?service=notify_verify&partner=2088221704766205&notify_id=c8b3e34f802467594cec7af1385e875i8m
		) {
			return true;
		} else {
			logger.error("支付宝回调验证失败, 验证参数:{}, sign:{}, response:{}", params, isSign, responseTxt);
			return false;
		}
	}
	 ...
	 * @return 验证结果
	 */
	public static boolean verifyMd5(Map<String, String> params) {
		logger.debug("开始进行支付宝回调有效性验证, 回调参数:{}", params);
		// 判断responsetTxt是否为true，isSign是否为true
		// responsetTxt的结果不是true，与服务器设置问题、合作身份者ID、notify_id一分钟失效有关
		// isSign不是true，与安全校验码、请求时的参数格式（如：带自定义参数等）、编码格式有关
		...
		boolean isSign = getSignVeryfyMd5(params, sign);
		logger.debug("isSign:{}", isSign);
		// 写日志记录（若要调试，请取消下面两行注释）
		// String sWord = "responseTxt=" + responseTxt + "\n isSign=" + isSign + "\n 返回回来的参数：" +
		// AlipayCore.createLinkString(params);
		// AlipayCore.logResult(sWord);
	 ...
	 * @return 生成的签名结果
	 */
	private static boolean getSignVeryfyMd5(Map<String, String> Params, String sign) {
		// 过滤空值、sign与sign_type参数
		Map<String, String> sParaNew = AlipayCore.paraFilter(Params);
		// 获取待签名字符串
		String preSignStr = AlipayCore.createLinkString(sParaNew);
